...
process_diet <- function(
  ...
  df_clean1 <- df_crossectional %>%
    select(Patient.ID, PID, Array, Collection.Date, all_of(old_columns), Main.Diet, Supplements.Medication, Anti.Aging.Interventions)
  ...
  condition_unsensitive <- grepl(searchwords_case_unsensitive, df_clean1$Supplements.Medication, ignore.case = TRUE) |
    ...
    grepl(searchwords_case_unsensitive, df_clean1$Main.Diet, ignore.case = TRUE)
  ...
  if (searchwords_case_sensitive != "") {
    condition_sensitive <- grepl(searchwords_case_sensitive, df_clean1$Supplements.Medication, ignore.case = FALSE) |
      ...
      grepl(searchwords_case_sensitive, df_clean1$Main.Diet, ignore.case = FALSE)
  } else {
  ...
  df_clean <- df_clean1 %>%
    select(-all_of(old_columns), -Supplements.Medication, -Anti.Aging.Interventions, -Main.Diet)
