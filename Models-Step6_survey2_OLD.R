...
print(colnames(df1_imp_data_surv2))


# # For all columns in your dataset, calculate missing percentage
# missing_summary <- sapply(df1_imp_data_surv2, function(x) mean(is.na(x)) * 100)
...
# Identical_summary <- sapply(df1_imp_data_surv2, function(x) length(unique(x)) == 1)
