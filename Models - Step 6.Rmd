...
remove_terms_if_p_large <- function(terms, dfs) {
  remaining_terms <- c()
  for (term in terms) {
    # Check if p.value is more than 0.15 in all dataframes
    all_over_15 <- all(sapply(dfs, function(df) {
      row_match <- df[df$term == term, ]
      if(nrow(row_match) == 0 || is.na(row_match$p.value)) {
        FALSE
      } else {
        all(row_match$p.value > 0.15)
      }
    }))
    if(!all_over_15) {
...
interventions_updated <- remove_terms_if_p_large(
    interventions, 
    list(DunedinPACE_Model1_Survey1, 
        GrimAge_PCAgeDev_Model2_Survey1, 
        OMICmAgeAgeDev_Model1_Survey1))
...
model3_GrimAge_PCAgeDev<-fit_imputed_lm(imp_data_surv1, outcome = "GrimAge.PCAgeDev", exposures=interventions, covariates=covariates_to_always_include)
# Save GrimAge.PCAgeDev plot
