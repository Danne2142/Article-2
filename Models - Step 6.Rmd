...
lifestyle_covariates_survey1<-c("Alcohol_per_week_numeric", "Marital.Status", 
"Education_levels_numeric", "Stress.Level", "Caffeine.Use", "Tobacco.Use.Numeric", 
"Main.Diet", "Exercise.per.week_numeric", "Exercise.Type", "Sexual.Frequency", 
"Hours.of.sleep.per.night", "Lab.Fasting", "Main.Diet.Factor" , "BMI")
...
remove_terms_if_p_large <- function(terms, dfs, categorical_variables_to_exclude = NULL) {
  ...
  # Process each term
  for (term in terms) {
    # Validate: check term exists and has valid p-values in all dataframes
    for (df in dfs) {
      row_match <- df[df$term == term, ]
      if (nrow(row_match) == 0) {
        stop(sprintf("ERROR: Term '%s' not found in one or more dataframes", term), call. = FALSE)
      }
      if (is.na(row_match$p.value)) {
        stop(sprintf("ERROR: p-value for term '%s' is NA in one or more dataframes", term), call. = FALSE)
      }
    }
...
lifestyle_covariates_survey1_updated <- remove_terms_if_p_large(
  lifestyle_covariates_survey1, categorical_variables_to_exclude=c("Main.Diet.Factor", "Excercise.Type"),
...
combined_results_df<-model_exposure_wise(path="C:/Users/danwik/OneDrive - Karolinska Institutet/Documents/Project 2 - Vscode/Output/imputed_survey1.RData", outcome_vars=c("DunedinPACE", "GrimAge.PCAgeDev", "OMICmAgeAgeDev"), exposure_vars=interventions_large_p, covariate_vars = covariates_to_always_include)
