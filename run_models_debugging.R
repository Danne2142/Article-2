...
if (saveModel1 == TRUE) {
  ...
  p1 <- plot_forest(combined_results_df[["DunedinPACE"]],
    ylab = "Term", xlab = " Aging Pace (biological year/chronological year)")
  ggsave(paste0(savePath, "DunedinPACE_Model1_Survey1_plot", suffix, ".png"),
    plot = p1)

  # Save GrimAge.PCAgeDev plot
  p2 <- plot_forest(combined_results_df[["GrimAge.PCAgeDev"]],
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "GrimAge_PCAgeDev_Model1_Survey1_plot", suffix, ".png"),
    plot = p2)

  # Save OMICmAgeAgeDev plot
  p3 <- plot_forest(combined_results_df[["OMICmAgeAgeDev"]],
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "OMICmAgeAgeDev_Model1_Survey1_plot", suffix, ".png"),
    plot = p3)
}
# Save as dataframes
...
if (saveModel1SD == TRUE) {
  ...
  fused_plot_model1 <- forestplot_fusion(
    DunedinPACE, GrimAge, OmicAge,
    source_names = c("DunedinPACE", "GrimAge", "OmicAge"),
    color_values = c("darkred", "navy", "forestgreen"),
    ylab = "Term", xlab = "SD")

  ggsave(paste0(savePath, "fused_Model1SD_Survey1_plot", suffix, ".png"),
    plot = fused_plot_model1)
}
...
if (saveModel2 == TRUE) {
  ...
  p4 <- plot_forest(combined_results_df[["DunedinPACE"]],
    ylab = "Term", xlab = "Aging Pace (biological year/chronological year)")
  ggsave(paste0(savePath, "DunedinPACE_Model2_Survey1_plot", suffix, ".png"),
    plot = p4)

  p5 <- plot_forest(combined_results_df[["GrimAge.PCAgeDev"]],
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "GrimAge_PCAgeDev_Model2_Survey1_plot", suffix, ".png"),
    plot = p5)

  p6 <- plot_forest(combined_results_df[["OMICmAgeAgeDev"]],
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "OMICmAgeAgeDev_Model2_Survey1_plot", suffix, ".png"),
    plot = p6)
...
if (saveModel2SD == TRUE) {
  ...
  fused_plot_model2 <- forestplot_fusion(
    DunedinPACE, GrimAge, OmicAge,
    source_names = c("DunedinPACE", "GrimAge", "OmicAge"),
    color_values = c("darkred", "navy", "forestgreen"),
    ylab = "Term", xlab = "SD")

  ggsave(paste0(savePath, "fused_Model2SD_Survey1_plot", suffix, ".png"),
    plot = fused_plot_model2)
}
...
if (saveModel3 == TRUE) {
  model3_DunedinPACE <- fit_imputed_lm(
    ...
    covariates = covariates_to_always_include)
  DunedinPACE <- subset(model3_DunedinPACE, term %in% interventions_large_p)
  p7 <- plot_forest(DunedinPACE,
    ylab = "Term", xlab = "Aging Pace (biological year/chronological year)")
  ggsave(paste0(savePath, "DunedinPACE_Model3_Survey1_plot", suffix, ".png"),
    plot = p7)

  model3_GrimAge_PCAgeDev <- fit_imputed_lm(
    imp_data, outcome = "GrimAge.PCAgeDev",
    exposures = interventions_large_p,
    covariates = covariates_to_always_include)
  GrimAge <- subset(model3_GrimAge_PCAgeDev, term %in% interventions_large_p)
  p8 <- plot_forest(GrimAge,
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "GrimAge_PCAgeDev_Model3_Survey1_plot", suffix, ".png"),
    plot = p8)

  model3_OMICmAgeAgeDev <- fit_imputed_lm(
    imp_data, outcome = "OMICmAgeAgeDev",
    exposures = interventions_large_p,
    covariates = covariates_to_always_include)
  OmicAge <- subset(model3_OMICmAgeAgeDev, term %in% interventions_large_p)
  p9 <- plot_forest(OmicAge,
    ylab = "Term", xlab = "Age Deviation (years)")
  ggsave(paste0(savePath, "OMICmAgeAgeDev_Model3_Survey1_plot", suffix, ".png"),
    plot = p9)
}
...
if (saveModel3SD == TRUE) {
  model3_DunedinPACE <- fit_imputed_lm(
    ...
    covariates = covariates_to_always_include)
  DunedinPACE <- subset(model3_DunedinPACE, term %in% interventions_large_p)
  # Save as dataframes (if needed, adjust accordingly)
  DunedinPACE_Model3_Survey1 <- combined_results_df[["DunedinPACE_z"]]

  model3_GrimAge_PCAgeDev <- fit_imputed_lm(
    imp_data, outcome = "GrimAge.PCAgeDev_z",
    exposures = interventions_large_p,
    covariates = covariates_to_always_include)
  GrimAge <- subset(model3_GrimAge_PCAgeDev, term %in% interventions_large_p)

  model3_OMICmAgeAgeDev <- fit_imputed_lm(
    imp_data, outcome = "OMICmAgeAgeDev_z",
    exposures = interventions_large_p,
    covariates = covariates_to_always_include)
  OmicAge <- subset(model3_OMICmAgeAgeDev, term %in% interventions_large_p)

  fused_plot_model3 <- forestplot_fusion(
    DunedinPACE, GrimAge, OmicAge,
    source_names = c("DunedinPACE", "GrimAge", "OmicAge"),
    color_values = c("darkred", "navy", "forestgreen"),
    ylab = "Term", xlab = "SD")

  ggsave(paste0(savePath, "fused_Model3SD_Survey1_plot", suffix, ".png"),
    plot = fused_plot_model3)
}
